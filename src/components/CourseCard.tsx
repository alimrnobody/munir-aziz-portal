...
export const CourseCard = ({ course, index }: CourseCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="h-1 w-full gradient-neon rounded-full mb-5 opacity-80 group-hover:opacity-100 transition-opacity" />

      {/* Icon badge */}
      <div className="w-10 h-10 rounded-xl gradient-neon-subtle flex items-center justify-center mb-4 group-hover:neon-glow transition-shadow duration-500">
        <Zap size={18} className="text-primary" />
      </div>

      <div className="flex items-start justify-between mb-3">
        <NeonText as="h3" glow className="text-lg leading-tight">
          {course.title}
        </NeonText>
        <ChevronRight 
